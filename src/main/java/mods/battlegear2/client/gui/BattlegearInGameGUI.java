...
public class BattlegearInGameGUI extends Gui {
    ...
    public void renderGameOverlay(float frame, int mouseX, int mouseY) {
        ...
        if (Battlegear.battlegearEnabled && !this.mc.playerController.enableEverythingIsScrewedUpMode()) {
            ...
            if (mc.thePlayer != null) {
                if (((IBattlePlayer) mc.thePlayer).isInBattleMode() || BattlegearConfig.alwaysShowBattleBar) {
            ...
            ItemStack offhand = ((IBattlegearInventoryPlayer) mc.thePlayer.inventory).getCurrentOffhandWeapon();
            ...
            if (!quiverFound) {
                mainhand = ((IBattlegearInventoryPlayer) mc.thePlayer.inventory).getCurrentOffhandWeapon();
                if (mainhand != null) {
                    ItemStack quiver = QuiverArrowRegistry.getArrowContainer(mainhand, mc.thePlayer);
                    if (quiver != null) {
                        event = new RenderItemBarEvent.QuiverSlots(renderEvent, mainhand, quiver);
    ...
    public void renderBattleSlots(int x, int y, float frame, boolean isMainHand) {
        ...
        if (mc.thePlayer != null) {
            if (((IBattlePlayer) mc.thePlayer).isInBattleMode()) this.drawTexturedModalRect(
                    x + (mc.thePlayer.inventory.currentItem - OFFSET) * 20 - 1,
                    y - 1,
                    0,
                    22,
                    24,
